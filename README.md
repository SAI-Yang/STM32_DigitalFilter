...
基于 STM32F103C8T6 的 FIR 低通数字滤波器课程设计项目。

**核心流程**: 信号输入 → ADC 采样 (10kHz) → DMA 传输 → FIR 低通滤波 → Goertzel 幅值分析 → 串口输出 + OLED 显示

> **最新测试 (2026-05-10)**: 500Hz 信号几乎无损通过 (~0dB), 3000Hz 信号衰减 >52dB, 滤波器性能优秀。详见 [CHANGELOG.md](CHANGELOG.md)
...
### 实测结果 (2026-05-10 实物, 软件测试信号)

| 频点 | 滤波前 | 滤波后 | 衰减 | 说明 |
|------|--------|--------|------|------|
| 500 Hz | 1.000V | 0.998V | ~0dB | 通带内, 几乎无衰减 |
| 3000 Hz | 0.500V | 0.001V | **-52.3dB** | 阻带内, 衰减优良 |

### 串口输出示例

```
426  500Hz   1.000V->0.978V    -0.2dB
426  3kHz    0.500V->0.001V   -52.3dB
427  500Hz   1.000V->0.979V    -0.2dB
427  3kHz    0.500V->0.001V   -51.2dB
```
